-- ============================================================
-- 105_seed_volunteer_attendance_dev.sql
-- dev 서버용 — church_id=19715 (행복샘교회 2). local 버전은 105_seed_volunteer_attendance.sql(church_id=33632) 참고.
-- 봉사 참여 출결 테스트 데이터 (reg_service_attendance)
--
-- 대상 church_id : 19715 (104_seed_volunteer_dev.sql 선행 필요 — 팀 8건/매핑 30건)
-- reg_service_attendance는 church_id 컬럼이 없고 service_team_id/member_id로만 참조하므로
-- 데이터 내용 자체는 local 버전과 동일함 (104_seed_volunteer_dev.sql이 먼저 적재되어 있어야 FK 정합성 유지).
