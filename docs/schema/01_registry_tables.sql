...
-- ================================================================
-- 12. 교육 과정 템플릿 (reg_education_courses)
-- ================================================================
DROP TABLE IF EXISTS `reg_education_courses`;
CREATE TABLE `reg_education_courses` (
    `id`               BIGINT       NOT NULL AUTO_INCREMENT,
    `church_id`        BIGINT       NOT NULL,
    `name`             VARCHAR(100) NOT NULL COMMENT '과정명 (새가족반/제자훈련 등)',
    `description`      TEXT         NULL     COMMENT '과정 설명',
    `target`           VARCHAR(50)  NULL     COMMENT '교육 대상: member | newcomer',
    `edu_type`         VARCHAR(50)  NULL     COMMENT '교육 유형: newcomer | nurture | training | other',
    `total_weeks`      TINYINT      NULL     COMMENT '기본 기간(주) — 기수 종료일 자동 계산 기본값',
    `total_rounds`     SMALLINT     NULL     COMMENT '기본 횟수(회) — 진도율 계산 기본값',
    `use_attendance`   TINYINT      NOT NULL DEFAULT 1 COMMENT '출결 관리 사용 여부 (1=사용)',
    `instructor`       VARCHAR(100) NULL     COMMENT '담당자 (기수 없을 때 목록 표시용)',
    `status`           ENUM('upcoming','ongoing','completed') NULL COMMENT '운영 상태 (기수 없을 때 표시용)',
    `start_date`       DATE         NULL     COMMENT '시작일 (기수 없을 때 표시용)',
    `end_date`         DATE         NULL     COMMENT '종료일 (기수 없을 때 표시용)',
    `completion_rate`  TINYINT      NOT NULL DEFAULT 80  COMMENT '수료 최소 출석률(%)',
    `repeat_mode`      VARCHAR(20)  NULL     DEFAULT 'annual' COMMENT 'annual|ondemand',
    `auto_complete`    TINYINT      NOT NULL DEFAULT 0   COMMENT '수료 자동 처리 여부',
    `recommendation`   TEXT         NULL     COMMENT '추천 대상 설명',
    `created_at`       TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at`       TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    KEY `idx_church_id` (`church_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='교육 과정 템플릿';


-- ================================================================
-- 13. 교육 기수 (reg_education_sessions)
-- ================================================================
DROP TABLE IF EXISTS `reg_education_sessions`;
CREATE TABLE `reg_education_sessions` (
    `id`           BIGINT       NOT NULL AUTO_INCREMENT,
    `course_id`    BIGINT       NOT NULL COMMENT 'reg_education_courses.id',
    `church_id`    BIGINT       NOT NULL,
    `generation`   VARCHAR(50)  NOT NULL COMMENT '기수명 (1기, 2025상반기 등)',
    `instructor`   VARCHAR(100) NULL     COMMENT '담당자',
    `start_date`   DATE         NULL     COMMENT '시작일',
    `end_date`     DATE         NULL     COMMENT '종료일',
    `capacity`     SMALLINT     NULL     COMMENT '정원',
    `total_rounds` SMALLINT     NULL     COMMENT '실제 총 횟수 (과정 기본값 오버라이드)',
    ...
    KEY `idx_church_id` (`church_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='교육 기수 (실제 운영 단위)';


-- ================================================================
-- 14. 교육 수강 기록 (reg_education_records)
-- ================================================================
DROP TABLE IF EXISTS `reg_education_records`;
CREATE TABLE `reg_education_records` (
    `id`          BIGINT     NOT NULL AUTO_INCREMENT,
