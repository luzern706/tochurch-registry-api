-- ================================================================
-- dev 서버용 — church_id=19715 (행복샘교회 2). local 버전은 100_seed_members_50.sql 참고.
--
-- ⚠️ 주의: 101_seed_registry_full.sql 을 이미 dev(church_id=19715)에 적재했다면
--    실제 교인 53명이 이미 존재한다. 이 파일은 member_no(M-2026001~050)가
--    전역 UNIQUE라 101의 데이터와 겹치면 INSERT IGNORE로 조용히 스킵된다.
--    101을 이미 적용했다면 이 파일은 보통 불필요 — 완전히 새 교회(빈 DB)에
--    최소 데이터로 테스트하고 싶을 때만 사용할 것.
--
-- 교인 50명 테스트 데이터
-- DB: gyohyero_db
